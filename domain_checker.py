...
def check_domain_availability(domain):
    ...
    try:
        ...
        if domain_info.domain_name:  # type: ignore
        ...
        # Domain is likely available if whois lookup fails with WhoisError
        # (typically means domain not found in registry)
        logger.debug(f"WhoisError for {domain} (likely available): {e}")
        return True, 'available'
